...
_(Pending — will be updated after Stage 2 fine-tuning completes)_

## Comparison with Non-Instruct Model

| Stage | Non-Instruct (Colab) | Instruct (RTX 5090) |
|-------|---------------------|---------------------|
| Base | 0.8612 | 0.8116 |
| Stage 1 | 0.9426 | 0.8956 |
| Stage 2 | 0.9465 | _(pending)_ |

The non-instruct model outperforms due to:
1. **bf16 compatibility** on Colab GPUs → batch 64-128 (vs batch 16 in fp32)
2. **More in-batch negatives** (63-127 vs 15) → stronger MNRL training signal
3. **Gradient checkpointing works** on non-Blackwell GPUs → larger batches possible
