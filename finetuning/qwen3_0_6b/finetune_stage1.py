"""
Fine-tune Qwen3-Embedding-0.6B on EU AI Act query-chunk pairs
using Matryoshka + MNRL.

Key differences from e5-large:
  - Decoder architecture with last-token pooling (not mean pooling)
  - Instruct format: "Instruct: {task}\\nQuery:{query}" for queries
  - No prefix for documents
  - bf16 + flash_attention_2 works on Blackwell (unlike e5-large)
...
def detect_device():
    ...
    Qwen3 on Blackwell supports bf16 + flash_attention_2
    (unlike e5-large which required fp32 + eager).
    """
...
def load_model(model_name, card_name="Qwen3-Embedding-0.6B EU AI Act NL Matryoshka"):
...
if __name__ == "__main__":
    ...
    training_args = SentenceTransformerTrainingArguments(
        ...
        run_name="qwen3-0.6b-dutch-regulations-mnrl",
        report_to="wandb",
    )
